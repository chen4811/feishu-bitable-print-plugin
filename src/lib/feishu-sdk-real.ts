...
export const feishuSDK = {
  ...
  /**
   * 获取当前选中的记录 ID 列表（表格视图复选框选中的记录）
   * 这是表格第一列复选框选中的记录，不是复选框字段
   * @returns 选中的记录 ID 数组
   */
  async getSelectRecordIds(): Promise<string[]> {
    console.log('[FeishuSDK] ========== getSelectRecordIds() 开始 ==========');
    ...
    try {
      ...
      console.log('[FeishuSDK] bitable.ui 存在:', !!bitable.ui);
      console.log('[FeishuSDK] bitable.ui 可用方法:', 
        bitable.ui ? Object.keys(bitable.ui).filter(k => typeof (bitable.ui as any)[k] === 'function') : []);
      ...
      // 方法1: bitable.ui.getSelectRecordIds()
      if (typeof bitable.ui.getSelectRecordIds === 'function') {
        console.log('[FeishuSDK] 调用 bitable.ui.getSelectRecordIds()');
        try {
          selectedIds = await bitable.ui.getSelectRecordIds();
          console.log('[FeishuSDK] getSelectRecordIds() 返回:', selectedIds);
        } catch (e) {
          console.log('[FeishuSDK] getSelectRecordIds() 调用失败:', e);
        }
      }
      
      // 方法2: 如果上面返回空，尝试从 getSelection 获取单条
      if (selectedIds.length === 0) {
        console.log('[FeishuSDK] 尝试从 getSelection 获取');
        const selection = await this.getSelection();
        if (selection?.recordId) {
          selectedIds = [selection.recordId];
          console.log('[FeishuSDK] 从 getSelection 获取到:', selectedIds);
        }
      }
      
      console.log('[FeishuSDK] ========== getSelectRecordIds() 结束，返回:', selectedIds);
      return selectedIds;
      
    } catch (error) {
      console.error('[FeishuSDK] ❌ getSelectRecordIds() 失败:', error);
      return [];
    }
  },
  ...
  /**
   * 监听表格复选框选择变化
   * 当用户在表格视图中勾选/取消勾选复选框时触发
   * @param callback 选择变化时的回调函数，参数为选中的记录 ID 数组
   * @returns 取消监听的函数
   */
  onSelectRecordChange(callback: (recordIds: string[]) => void): () => void {
    console.log('[FeishuSDK] ========== onSelectRecordChange 注册监听 ==========');
    ...
    const setupListener = async () => {
      try {
        ...
        if (!bitable || !bitable.ui) {
          console.log('[FeishuSDK] bitable.ui 不存在，无法监听选择变化');
          return;
        }
        
        console.log('[FeishuSDK] bitable.ui 可用方法:', 
          Object.keys(bitable.ui).filter(k => typeof (bitable.ui as any)[k] === 'function'));
        
        // 尝试注册监听器
        if (typeof bitable.ui.onSelectRecordChange === 'function') {
          console.log('[FeishuSDK] 注册 bitable.ui.onSelectRecordChange');
          unsubscribeFn = bitable.ui.onSelectRecordChange(async (event: any) => {
            if (isUnsubscribed) return;
            console.log('[FeishuSDK] 🎯 选择变化事件触发:', event);
            
            // 获取当前选中的记录 ID
            const selectedIds = await this.getSelectRecordIds();
            console.log('[FeishuSDK] 当前选中记录:', selectedIds);
            callback(selectedIds);
          });
          console.log('[FeishuSDK] ✅ 监听器注册成功');
        } else {
          console.log('[FeishuSDK] ⚠️ onSelectRecordChange 方法不存在');
          
          // 备选方案：轮询检测变化
          console.log('[FeishuSDK] 使用轮询方式检测变化');
          let lastIds: string[] = [];
          const pollInterval = setInterval(async () => {
            ...
            try {
              const currentIds = await this.getSelectRecordIds();
              if (JSON.stringify(currentIds) !== JSON.stringify(lastIds)) {
                console.log('[FeishuSDK] 轮询检测到变化:', { lastIds, currentIds });
                lastIds = currentIds;
                callback(currentIds);
              }
            } catch (e) {
          ...
          unsubscribeFn = () => {
            clearInterval(pollInterval);
            console.log('[FeishuSDK] 轮询已停止');
          };
        }
        
      } catch (error) {
        console.error('[FeishuSDK] 设置选择监听失败:', error);
      }
    };
